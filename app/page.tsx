...
export default function Home() {
  ...
  const submitCode = () => {
    ...
    if (!master) { setResult(correct ? 'win' : 'wrong'); return; }
...
function RulesMenu({ locale, open, onToggle, onOpenRules, onOpenReference }: { locale: Locale; open: boolean; onToggle: () => void; onOpenRules: () => void; onOpenReference: () => void }) { const t = (text: string) => ui(locale, text); return <div className="rules-menu-wrap"><button className="rules-button" onClick={onToggle} aria-expanded={open} aria-haspopup="menu">{t('规则')} <i className="rules-chevron" aria-hidden="true" /></button>{open && <div className="rules-menu" role="menu"><button role="menuitem" onClick={onOpenRules}><span><strong>{t('规则介绍')}</strong><small>{t('查看完整网页版规则书')}</small></span><em>→</em></button><button role="menuitem" onClick={onOpenReference}><span><strong>{t('标准卡速查')}</strong><small>{t('查看全部 48 张标准卡')}</small></span><em>→</em></button></div>}</div>; }
...
function RuleBook({ onClose }: { onClose: () => void }) {
  ...
  return <div className="overlay rule-book-overlay"><section className="rule-book" role="dialog" aria-modal="true" aria-label={`规则介绍，第 ${page + 1} 页，共 ${ruleSlides.length} 页`}>
    <button className="rule-book-close" onClick={onClose} aria-label="关闭规则介绍"><i aria-hidden="true"/></button>
    <button className="rule-book-nav rule-book-prev" onClick={() => setPage((current) => Math.max(0, current - 1))} disabled={page === 0} aria-label="上一页"><i aria-hidden="true"/></button>
    <figure><img src={ruleSlides[page]} alt={`图灵机规则介绍第 ${page + 1} 页`}/></figure>
    <button className="rule-book-nav rule-book-next" onClick={() => setPage((current) => Math.min(ruleSlides.length - 1, current + 1))} disabled={page === ruleSlides.length - 1} aria-label="下一页"><i aria-hidden="true"/></button>
  </section></div>;
}
...
function VerifierReference({ locale, onClose }: { locale: Locale; onClose: () => void }) {
  ...
  return <div className="overlay result-overlay"><section className="verifier-reference" role="dialog" aria-modal="true" aria-labelledby="verifier-reference-title">
    <div className="reference-head"><div><p className="eyebrow">{locale === 'en' ? 'CRITERIA CARD REFERENCE' : 'STANDARD CARD REFERENCE'}</p><h2 id="verifier-reference-title">{ui(locale, '标准卡速查')}</h2><p>{ui(locale, '全部 48 张标准卡的问题、可能标准与规则书说明。速查内容是公开信息，不会显示当前题目的隐藏答案。')}</p></div><button onClick={onClose} aria-label={locale === 'en' ? 'Close Criteria Card reference' : '关闭标准卡速查'}>×</button></div>
    ...
    <button className="reference-close" onClick={onClose}>{ui(locale, '关闭速查')}</button>
  </section></div>;
}
